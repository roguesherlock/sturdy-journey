...
import { SocketStates, PhoenixContext, SocketHook } from "src/types/phoenix";

type SocketOpts = typeof SocketConnectOption & {
  onOpen: () => void;
  onClose: () => void;
...
export function usePhoenixSocket(url: string, opts?: Partial<SocketOpts>) {
  ...
  const socketOnMessageCallback = useCallback(
    (callback) => socketRef.current.onMessage(callback),
    [socketRef]
  );
