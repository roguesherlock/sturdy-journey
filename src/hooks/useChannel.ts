...
import { ChannelHook, ChannelStates } from "src/types/phoenix";
...
export function useChannel(
  ...
  if (socket === undefined)
    throw new Error(
      "Socket is undefined. Is component within a provider or has a socket been passed?"
    );
  ...
  useEffect(() => {
    ...
    channelRef.current
      .join()
      .receive("ok", (args: any) => {
        if (typeof onJoin === "function") onJoin(args);
  ...
  const handleEventCallback = useCallback(
    (event, callback) => channelRef.current.on(event, callback),
    []
  );

  const pushEventCallback = useCallback((event, payload, timeout?) => {
    // @ts-ignore
